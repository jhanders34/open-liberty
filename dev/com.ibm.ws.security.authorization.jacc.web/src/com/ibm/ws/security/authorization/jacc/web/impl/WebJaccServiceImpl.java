...
public class WebJaccServiceImpl implements WebJaccService {
    ...
    static {
        ...
        if (principalMapperSupported) {
            jaccHandlerKeyArray = new String[] { "javax.security.auth.Subject.container", "jakarta.servlet.http.HttpServletRequest", "jakarta.security.jacc.PrincipalMapper" };
        } else {
            // javax.servlet.http prefixed string will be jakarta-ized during transformation
    ...
    private boolean checkResourceConstraints(String contextId, HttpServletRequest req, Permission webPerm, Subject subject, PolicyProxy policyProxy) {
        ...
        try {
            ...
            result = checkResourceConstraints(contextId, req, webPerm, subject, ho, policyProxy);
        } catch (PrivilegedActionException e) {
            Tr.error(tc, "JACC_WEB_IMPLIES_FAILURE", new Object[] { contextId, e.getException() });
        }
        return result;
